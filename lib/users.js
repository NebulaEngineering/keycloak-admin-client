 ...
 */
function checkTokenValidity(client) {
  return function checkTokenValidity(realmName) {
    return new Promise((resolve, reject) => {
      const req = {
        ...
        json: true,
        url: `${client.baseUrl}/realms/${realmName}/protocol/openid-connect/userinfo`
      };
  ...
  return function resetPassword(realmName, userId, payload) {
    return new Promise((resolve, reject) => {
      ...
      const req = {
        ...
        auth: {
          bearer: privates.get(c).token.access_token
        },
        json: true,
        method: 'PUT',
        body: payload
...
/**
  A function to assign a group to an user for a realm
  @param {string} realmName - The name of the realm(not the realmID) - ex: master,
...
function addGroup(client) {
  return function addGroup(realmName, userId, groupId) {
    return new Promise((resolve, reject) => {
      const req = {
...
/**
  A function to de-assign a group to an user for a realm
  @param {string} realmName - The name of the realm(not the realmID) - ex: master,
  @param {String} userId - user id
  @param {String} groupId - the group id
  @returns {Promise} A promise that resolves.
 ...
 */
function removeGroup(client) {
  return function removeGroup(realmName, userId, groupId) {
    return new Promise((resolve, reject) => {
      const req = {
        url: `${client.baseUrl}/admin/realms/${realmName}/users/${userId}/groups/${groupId}`,
        auth: {
